...
def cifar_shakeshake26(pretrained=False, **kwargs):
    ...
    #model = ResNet32x32_spec_norm(ShakeShakeBlock,
    #                    layers=[4, 4, 4],
    #                    channels=96,
    #                    downsample='shift_conv', **kwargs)                        
...
class WideResNet(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, dropout=0.2, dense_dropout=0.2):
        ...
        self.bn2 = nn.BatchNorm2d(channels[2], momentum=0.001)
        self.bn3 = nn.BatchNorm2d(channels[3], momentum=0.001)
    ...
    def forward(self, x):
        ...
        return self.fc1(self.drop(out)), self.fc2(self.drop(out))
...
class ResNet32x32_spec_norm(nn.Module):
    def __init__(self, block, layers, channels, groups=1, num_classes=1000, downsample='basic'):
        super().__init__()
        assert len(layers) == 3
        self.downsample_mode = downsample
        self.inplanes = 16
        self.conv1 = torch.nn.utils.parametrizations.spectral_norm(nn.Conv2d(3, 16, kernel_size=3, stride=1,
                                                                   padding=1, bias=False))
        self.layer1 = self._make_layer(block, channels, groups, layers[0])
        ...
        self.avgpool = nn.AvgPool2d(8)
        self.fc1 = torch.nn.utils.parametrizations.spectral_norm(nn.Linear(block.out_channels(channels * 4, groups), num_classes))
        self.fc2 = torch.nn.utils.parametrizations.spectral_norm(nn.Linear(block.out_channels(channels * 4, groups), num_classes))
    ...
    def _make_layer(self, block, planes, groups, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != block.out_channels(planes, groups):
            if self.downsample_mode == 'basic' or stride == 1:
                downsample = nn.Sequential(
                    torch.nn.utils.parametrizations.spectral_norm(nn.Conv2d(self.inplanes, block.out_channels(planes, groups),
                                                                  kernel_size=1, stride=stride, bias=False)),
                    nn.BatchNorm2d(block.out_channels(planes, groups)),
                )
            elif self.downsample_mode == 'shift_conv':
...
class ShiftConvDownsample(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.relu = nn.ReLU(inplace=True)
        self.conv = torch.nn.utils.parametrizations.spectral_norm(nn.Conv2d(in_channels=2 * in_channels,
